...
class MobileActivityDashboard:
    ...
    def setup_routes(self):
        ...
        def get_status():
            """Get system status"""
            activities_count = len(self.get_recent_activities(24))
            pending_count = len(self.get_pending_changes())

            status = {
                "status": "operational",
                "activities_24h": activities_count,
    ...
    def approve_code_change(self, change_id: int):
        ...
        try:
            ...
            cursor.execute(
                """
                UPDATE code_changes 
                SET status = 'approved', user_approved = TRUE
                WHERE id = ?
            """,
                (change_id,),
            )
    ...
    def reject_code_change(self, change_id: int):
        ...
        try:
            ...
            cursor.execute(
                """
                UPDATE code_changes 
                SET status = 'rejected', user_approved = FALSE
                WHERE id = ?
            """,
                (change_id,),
            )
    ...
    def run(self, host="0.0.0.0", port=5000):
        ...
        print(f"🌐 Enhanced Mobile Dashboard starting on {host}:{port}")
        print("📱 Accessible from any network")
        print("📊 Real-time activity logging enabled")
...
<body>
    ...
    <div class="tabs">
        ...
        <button class="tab" onclick="showTab('pending')">Approvals</button>
    ...
    <div class="content">
        ...
        <div id="pending-tab" class="tab-content" style="display: none;">
            <div id="pending-list" class="loading">Loading pending approvals...</div>
        </div>
        ...
        function loadPending() {
            fetch('/api/pending')
                ...
                .then(pending => {
                    ...
                    container.innerHTML = pending.map(change => `
                        <div class="activity-item pending-item">
                            <div class="activity-header">
                                <span class="activity-type">Code Change</span>
                                <span class="activity-time">${formatTime(change.timestamp)}</span>
                            </div>
                            <div class="activity-description">${change.description}</div>
                            <div class="activity-details">
                                <strong>File:</strong> ${change.file_path}<br>
                                <strong>Old Code:</strong><br><code>${change.old_code}</code><br>
                                <strong>New Code:</strong><br><code>${change.new_code}</code>
                            </div>
                ...
                .catch(error => {
                    document.getElementById('pending-list').innerHTML = '<div class="loading">Error loading pending approvals</div>';
                });
        }
